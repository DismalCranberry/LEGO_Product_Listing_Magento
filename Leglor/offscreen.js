...
function findRoot(doc) {
  ...
  return stacks.find(el => el.querySelector("p") && FIELDS.some(f => el.textContent?.includes(f)))
      || doc.body;
}
...
function extractFromBlock(doc, labelP) {
  let block = labelP.closest('div[class*="css-hboir"]')
...
function extractFromHTML(html, url) {
  ...
  const out = {
    ...
    name: "", shopperName: "",
    titleShort: "", titleMedium: "", titleLong: "",
    header: "", quickView: "", description: "",
    bulletShort: [], bulletLong: [],
    url
  };
